...
function AdminActions({
  ...
  hostel: AdminHostel;
  onAction: (hostelId: string, action: "verify" | "suspend" | "activate") => void;
  loading: boolean;
}) {
  const btn = (
  ...
  const actionMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await fetch("/api/admin/hostels", {
        ...
        body:    JSON.stringify({ hostelId, action }),
      });
        ...
        <>
          <div className="rounded-[var(--radius-lg)] border border-[var(--color-border-subtle)] bg-[var(--color-bg-card)] overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[640px]" aria-label="Hostel listings">
                <thead>
                ...
                <tbody>
                  {hostels.map((hostel) => (
                    ...
                    >
                      ...
                      {/* Inline actions — no modal, per spec */}
                      <td className="px-4 py-3.5">
                        <AdminActions
                          hostel={hostel}
                          onAction={(hostelId, action) =>
                            actionMutation.mutate({ hostelId, action })
                          }
